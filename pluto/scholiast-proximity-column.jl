...
# ╔═╡ 775002f8-308b-11ee-313a-9b137cd4f64e
begin
	using JuMP
	using HiGHS
	using Luxor
...
# ╔═╡ 7536a2e1-da55-41ae-a278-dbd82c1cfae6
md"*To see the julia environment, unhide the following cell.*"

# ╔═╡ dfcc8f4b-02b8-45ea-894b-b925abc87f12
md"""### Add scholion to line


1. $(@bind ln1 NumberField(0.0:0.02:1.0))
1. $(@bind ln2 NumberField(0.0:0.02:1.0))
1. $(@bind ln3 NumberField(0.0:0.02:1.0))
1. $(@bind ln4 NumberField(0.0:0.02:1.0))
1. $(@bind ln5 NumberField(0.0:0.02:1.0))
1. $(@bind ln6 NumberField(0.0:0.02:1.0))
1. $(@bind ln7 NumberField(0.0:0.02:1.0))
1. $(@bind ln8 NumberField(0.0:0.02:1.0))
1. $(@bind ln9 NumberField(0.0:0.02:1.0))
1. $(@bind ln10 NumberField(0.0:0.02:1.0))
1. $(@bind ln11 NumberField(0.0:0.02:1.0))
1. $(@bind ln12 NumberField(0.0:0.02:1.0))
1. $(@bind ln13 NumberField(0.0:0.02:1.0))
1. $(@bind ln14 NumberField(0.0:0.02:1.0))
1. $(@bind ln15 NumberField(0.0:0.02:1.0))
1. $(@bind ln16 NumberField(0.0:0.02:1.0))
1. $(@bind ln17 NumberField(0.0:0.02:1.0))
1. $(@bind ln18 NumberField(0.0:0.02:1.0))
1. $(@bind ln19 NumberField(0.0:0.02:1.0))
1. $(@bind ln20 NumberField(0.0:0.02:1.0))
1. $(@bind ln21 NumberField(0.0:0.02:1.0))
1. $(@bind ln22 NumberField(0.0:0.02:1.0))
1. $(@bind ln23 NumberField(0.0:0.02:1.0))
1. $(@bind ln24 NumberField(0.0:0.02:1.0))
...
# ╔═╡ 63b86630-446b-41ac-a4b1-fdf22f14011c
md"""# Be a scholiast!

You have a page with $(length(heights)) lines of *Iliad* text, laid out with an adjacent column for notes (*scholia*). Add a scholion on a line by entering a value for the height of its text scaled between 0 and 1 where 0 means "no text" and 1 means "the full height of the page".

Sorry, only one scholion per line (unlike real manuscripts!)
"""
...
"Non-zero heights for scholia"
tgt_heights_pct = filter(h -> h > 0, heights)

# ╔═╡ ab40b76e-58c0-4440-ad8d-48ced419161d
"""Number of *Iliad* lines with scholia."""
n = length(tgt_heights_pct)


# ╔═╡ 67a87432-02c2-4744-9a7a-471949bb9808
"""Scholia heights in screen pixels"""
target_hts = page_h .* tgt_heights_pct

# ╔═╡ a11c8148-f976-4bcd-b331-5a8bd291089d
md"> **Graphics settings**"

# ╔═╡ 620748fc-e12e-4698-8431-ca98491002f0
page_w = .8 * page_h

# ╔═╡ 9e741e2d-e150-4ad7-bb48-fdbc73a0e54f
textbox = (top = 0.15, left = 0.1, bottom = 0.9, right = 0.7)

# ╔═╡ b810fed2-083a-492b-a5e7-3ca4f5119c71
textmidpt = (textbox[:left] + textbox[:right]) / 2

# ╔═╡ ccd18b4f-d074-43aa-8e1c-b7104e3fd9b2
notesbox = (top = 0.10, left = 0.75, right = 0.9, bottom = 0.95)

# ╔═╡ e8db099e-a1a8-4538-a88a-c90527a4a6b7
column_ceiling = notesbox[:top] * page_h 

# ╔═╡ 71eb8bb7-a9b1-47e1-9e63-f2fc36ed56c0
column_floor = notesbox[:bottom] * page_h
...
# ╔═╡ 51a8fba8-6da4-49b0-aa52-77efb2c4a924
notesmidpt = (notesbox[:left] + notesbox[:right]) / 2

# ╔═╡ 27aa52f0-f631-4e46-be37-00737c7781aa
palette = ColorSchemes.tableau_traffic

# ╔═╡ 41fee523-c91e-4f34-9a9b-cc727de05e82
"""Choose font size for current page width."""
function scaledfont()
	if page_h < 150
		6
	elseif page_h < 200
...
# ╔═╡ 180820e7-7fbc-4643-b41a-b19c3fc33380
"""Choose size of circle marking lines and scholia."""
function scaleddot()
	if page_w > 500
		4
	else
...
# ╔═╡ b1bd6469-69b9-4a5f-a756-51a96be8e455
md"""> **Luxor drawing functions**"""

# ╔═╡ 17c2f1f4-e05c-4a61-9f36-77e81afba357
"""Draw a box for the whole page frame"""	
function pageframe_luxor(x,y; luxaction = :stroke) 
	box(O, Point(x, y), action = luxaction)
end
...
function textframe_luxor(x,y; luxaction = :stroke)
	lt = Point(textbox[:left] * x, textbox[:top] * y)
	rb = Point(textbox[:right] * x, textbox[:bottom] * y)
	box(lt, rb, action = luxaction)
	
end

# ╔═╡ fbaefc09-3412-4476-8523-4ce4ace1e7df
"""Draw a box for the coordinates of notes column scaled to the
given `x` and `y` dimensions."""	
function notesframe_luxor(x,y; luxaction = :stroke)
	lt = Point(notesbox[:left] * x, notesbox[:top] * y)
	rb = Point(notesbox[:right] * x, notesbox[:bottom] * y)
	box(lt, rb, action = luxaction)
	
end

# ╔═╡ 59922b6f-25f2-47f5-9276-5c498875041f
"""Draw *Iliad* lines on diagram."""
function iliad_lines(; linehue = "gray", dotcolors = ColorSchemes.tol_light.colors, luxaction =  :stroke, nudge = 0.04)
	...
	for i in 1:lncount
		...
		pt_l = Point(textbox[:left] * page_w + page_w*nudge, y)
		pt_r = Point(textbox[:right] * page_w - page_w*nudge, y)
		sethue(linehue)
...
"""Compute scaled `y` value for all *Iliad* lines"""
function iliad_ys()
	lncount = length(heights)
	txtht = (textbox[:bottom] - textbox[:top]) * page_h
	spacing = txtht / (lncount + 1)
...
@draw begin
		...
		txty = textbox[:top] / 2
		titlept = Point(textmidpt * page_w, txty * page_h)
		fontsize(scaledfont())
		text("ΙΛΙΑΣ", titlept, halign = :center)
		
		
		sethue("skyblue")
		setdash("dot")
		pageframe_luxor(page_w, page_h)
...
julia_version = "1.9.1"
manifest_format = "2.0"
project_hash = "6f023b82b43b5d92797e01906e5565e52bdf0bcf"
