...
nonzero_annotations = filter(pr -> pr[1] > 0 && pr[2] > 0.0, annotations)
...
# ╔═╡ 61ba8239-c78d-46d5-829d-2475471157a0
target_heights_scaled = page_h .* target_heights

# ╔═╡ e721df57-6bfe-4063-8ca9-caac949956e7
md"> **Modeling the proximity hypothesis**"

# ╔═╡ 4849c8cf-7212-4ce4-94ab-a39e0f2c2a25
md"> **Graphics settings**"

# ╔═╡ 8fd87f6a-e122-4030-a5eb-28bc4286d140
page_w = .8 * page_h

# ╔═╡ 3283ca9b-ba27-4439-9007-49605bc162bc
textbox = (top = 0.15, left = 0.1, bottom = 0.9, right = 0.7)

# ╔═╡ 55efd572-a702-42ec-ab1e-7c08c4d9aa0e
textmidpt = ((textbox[:left] + textbox[:right]) / 2) * page_w

# ╔═╡ bfd00940-25eb-4e92-aba8-f238db3bde5a
textht = (textbox[:bottom] - textbox[:top]) * page_h

# ╔═╡ bd870da5-b902-461c-b3bf-001d103ce256
linespacing = textht / (linecount + 1)

# ╔═╡ 34db1187-3841-4389-8deb-305b3cb2cbaa
"Compute `y` coordinate of a given text line."
function scalelinenum(num)
	textbox[:top] * page_h + linespacing * num
end

# ╔═╡ 2f4641ff-2be7-4327-8bac-df07c3b71e12
target_ys_scaled = target_lines .|> scalelinenum

# ╔═╡ cbb11392-453f-45b6-9b43-3de52640c5dc
notesbox = (top = 0.10, left = 0.75, right = 0.9, bottom = 0.95)

# ╔═╡ 7465f390-6d72-4060-8b49-b5cca8ef2e0f
column_ceiling = notesbox[:top] * page_h 

# ╔═╡ 1d8d899b-4296-476d-8bf9-60db3734c4d6
column_floor = notesbox[:bottom] * page_h

# ╔═╡ 5e75795b-94ab-4dc0-b0cf-f8b96612fbb0
"""Compute optimal placement of notes with given heights to
...
# ╔═╡ 60a21d6c-5eaf-4836-9e2b-99b7d9658fed
...
"""Add text title to page"""
function title_luxor()
	txty = textbox[:top] / 2
	titlept = Point(textmidpt, txty * page_h)
	fontsize(scaledfont())
...
"""Draw *Iliad* lines on page."""
function iliad_lines_luxor(; luxaction =  :stroke, nudge = 0.04)
	for i in 1:linecount
		y = i * linespacing + textbox[:top] * page_h
...
md"> **Validating user input**"

# ╔═╡ 71a6f8ee-ccee-47fd-8188-4b97c4933c49
"List invalid line numbers"
function badheights()
	bad = []
	for pr in annotations
		if length(pr) != 2
			push!(bad, pr)
		elseif ! (0.0 <= pr[2] < 1.0)
			push!(bad, pr[2])
...
"True if annotation heights are valid."
function heightsok()
	isempty(badheights())
end

# ╔═╡ 08800dc1-1ba5-4d2a-9502-7d4bd3254083
"List invalid line numbers"
function badnums()
	bad = []
	for pr in annotations
		if ! (pr[1] in 0:linecount)
			push!(bad, pr[1])
		end
	end
	bad
...
# ╔═╡ 205ce125-66eb-4405-a044-75d95fb8e7b2
"True if line numbers are valid."
function linenumsok()
	isempty(badnums())
end

# ╔═╡ 50630b28-4b73-4c86-b4e0-77bfe6989aff
if  ! (typeof(annotations) <: Vector)
	danger(md"""Be sure to enclose your list in square brackets, like this:
...
elseif ! heightsok()
	danger(md"""The second number in each pair should be between 0 and 1.  Please fix your entry $(badheights())""")	
